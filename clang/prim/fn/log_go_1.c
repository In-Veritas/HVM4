...
fn Term prim_fn_log_go_1(Term *args) {
  ...
  switch (term_tag(head_wnf)) {
    ...
    case SUP: {
      ...
      app0 = term_new_app(app0, term_new_app(A.k0, con0));
      Term app1    = term_new_pri(table_find("log", 3));
      app1 = term_new_app(app1, term_new_app(A.k1, con1));
    ...
    case C00 ... C16: {
      if (term_tag(head_wnf) == C01 && term_ext(head_wnf) == NAM_CHR) {
        ...
        Term t = term_new_pri(table_find("log_go_2", 8));
        t = term_new_app(t, acc);
        t = term_new_app(t, code);
